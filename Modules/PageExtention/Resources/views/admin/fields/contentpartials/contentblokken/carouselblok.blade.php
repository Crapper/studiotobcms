@foreach($contentBlokCarousel as $image)
        ...
        @if($image->col_number == $i)
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <h3 class="panel-title">CarouselBlok</h3>
                    ...
                    <a class="pull-right bewerkImageBlockModal" id="bewerkImageBlockModal" data-toggle="modal" data-target="#bewerkImageBlockModal"
                       data-id="{{$image->id}}" data-page_id="{{$pageExtention->page_id}}" data-img_path="{{$image->img_path}}"
                       data-title="{{$image->title}}" data-image_text_body="{{$image->text_body}}"
                       data-button_label="{{$image->button_label}}" data-button_link="{{$image->button_link}}" data-standaard="{{$image->standaard}}" data-overlay="{{$image->overlay}}">
                ...
                <div class="panel-body content-blok-display">
                    ...
                    @if($image->overlay === 1)
                        <div class="content-blok-special-image" >
                            @if($image->title)<div class="afbeelding-titel"><h2>{{$image->title}}</h2></div>@endif
                    ...
                    @else
                        <div class="content-blok-special-image" >
                            @if($image->title)<div class="standaard-afbeelding-titel"><h2>{{$image->title}}</h2></div>@endif
